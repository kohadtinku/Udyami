...
if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST["addList"]) && $_POST['listing_title']) {
  ...
  $listing_pdf = "";
  if (isset($_FILES['listing_pdf'])) {
    
		if ("" != $_FILES["listing_pdf"]["tmp_name"]) {
			$listing_pdf = get_server_pdf_name('listing_pdf');
		}
	}

 include "../service/db.php";
  try {
    $insert_query = "INSERT INTO `listing` (`listing_id`, `listing_business_name`,`listing_firm_name`, `listing_business_category_id`, `listing_keyword`, `listing_tag`, `listing_cost`, `listing_business_sub_category_id`, `listing_description`, `listing_country_id`, `listing_pincode`, `listing_state_id`, `listing_location`, `listing_city_id`, `listing_image`,`listing_image2`,`listing_image3`,`listing_pdf`, `listing_timestamp`, `listing_status_timestamp`, `listing_status`, `listing_permission`, `listing_owner_id`) VALUES (NULL, '$listing_title','$listing_firm_name', '$category_id', '$keywords', '$tags', '$price', '$sub_category_id', '$listing_description', '1', '$pincode', '$state_id', '$address', '$city_id', '$logo_image1','$logo_image2','$logo_image3','$listing_pdf',  current_timestamp(),  current_timestamp(), 'Pending', 'Pending', '" . $_SESSION['user_id'] . "')";
...
?>
 ...
 <?php 
                ...
                $requested_sub_category_id = "";
                 if (isset($_GET["requested_category_id"])) {
                   $requested_category_id = $_GET['requested_category_id'];
                   ?>
                   
 <a href="./dashboard_add_listing.php" class="button  " style="margin-left:auto;float: right;"><i class="sl sl-icon-user"></i> Unlock All Categories and Sub-Categories</a>
               
        
                       
                <?php
                   //echo $requested_category_id;
                 }
                 ?>
              ...
              <form enctype="multipart/form-data" method="POST" action="<?= $_SERVER["REQUEST_URI"]; ?>">
                  ...
                  <div class="row with-forms">
                    ...
                    <div class="col-md-6">
                      ...
                      <div class="intro-search-field utf-chosen-cat-single">
                          ...
                          $select_req_category_query = "SELECT * FROM `category` where category_id =$requested_category_id"; //NOTE: here (`) is not equal to (')
                          try {
                            $select_req_category_result = 0;
                            if ($connect) {
                              $select_req_category_result = mysqli_query($connect, $select_req_category_query);
                              if ($select_req_category_result) {
                                $category_num = mysqli_num_rows($select_req_category_result);
                              }
                            }
                          } catch (Exception $e) {
                            $mess = $e->getMessage();
                          ...
                          if ($category_num > 0) {
                            ...
                            while ($row = mysqli_fetch_assoc($select_req_category_result)) {
                          $requested_category_id = $row["category_id"]; 
                          $requested_category_name = $row['category_name'] ; }
                          }
                          ?>
                          
                          
                              
                               <select class="default selectpicker " value="<?= $requested_category_id ?>" name="category_id" id="category" data-selected-text-format="count" data-size="7" title="<?= $requested_category_name?>" >

                        <option  value="<?php $requested_category_id; ?>" selected><?=  $requested_category_name ?></option>
                  ...
                  <div class="row with-forms">
                    ...
                    <div class="col-md-6">
                      ...
                      <div class="intro-search-field utf-chosen-cat-single">
                          ...
                          <?php 
                           $requested_sub_category_id = "";
                              if (isset($_GET["requested_sub_category_id"])) {
                                  
                                $requested_sub_category_id = $_GET['requested_sub_category_id'];
                                
                                //echo $requested_sub_category_id;
                              }


                          if($requested_sub_category_id != ''){
                                $select_req_sub_category_query = "SELECT * FROM `sub_category` where sub_category_id = $requested_sub_category_id"; //NOTE: here (`) is not equal to (')
                              try {
                                $select_req_sub_category_result = 0;
                                if ($connect) {
                                  $select_req_sub_category_result = mysqli_query($connect, $select_req_sub_category_query);
                                  if ($select_req_sub_category_result) {
                                    $sub_category_num = mysqli_num_rows($select_req_sub_category_result);
                                  }
                                }
                              } catch (Exception $e) {
                                $mess = $e->getMessage();
                              ...
                              if ($sub_category_num > 0) {
                                $sno = 0;
                                //Note : mysqli_fetch_assoc($result) it returns NULL when no data is persent to Select
                                while ($row = mysqli_fetch_assoc($select_req_sub_category_result)) {
                              $requested_sub_category_id = $row["sub_category_id"]; 
                              $requested_sub_category_name = $row['sub_category_name'] ; }
                              }
                          
                              ?>
                              
                              
                               <select class="default selectpicker" name="sub_category_id" value="<?= $requested_sub_category_id?>" style="margin-bottom:0px" id="sub-category" data-selected-text-format="count" data-size="7" title="<?= $requested_sub_category_name?>" >
                          
                          <option  value="<?php $requested_sub_category_id; ?>" selected><?=  $requested_sub_category_name ?></option>
                        ...
                        <select class="default " name="sub_category_id" value="" style="margin-bottom:0px" id="sub-category" data-selected-text-format="count" data-size="7" title="Select Sub-Category" >
                ...
                <div class="add_utf_listing_section margin-top-45">
                  ...
                  <div class="row with-forms">
                    ...
                    <div class="utf_submit_section col-md-4">
                      ...
                      <!-- <form></form> -->
                      <div class="">
                        <input type="file" name="listing_pdf" accept="application/pdf,application/vnd.ms-excel" />
                      </div>
                ...
                $requested_sub_category_id = "";
                 if (isset($_GET["requested_category_id"])) {
                   $requested_category_id = $_GET['requested_category_id'];
                   ?>
                    <input type="hidden"  name="category_id" value="<?= $requested_category_id?>" >
                ...
                <?php
                  ...
                  if (isset($_GET["requested_sub_category_id"])) {
                   ...
                   ?>
                    ...
                    <input type="hidden"  name="sub_category_id" value="<?= $requested_sub_category_id?>" >
                    <?php
                   //echo $requested_sub_category_id;
                 }
                 ?>
